 ...
 *
 * This module provides a WordListProvider implementation for the default profanity words list.
 * It fetches a curated list of profanity words from a remote URL with optional caching
 * and automatic refresh capabilities.
 *
 * @module DefaultWordListProvider
 */
 ...
 *
 * This provider fetches a curated list of profanity words from a remote GitHub repository.
 * By default, it uses the Google profanity words list from the coffee-and-fun/google-profanity-words
 * repository, but can be configured to use any URL. The provider supports caching to improve
 * performance and automatic refresh based on configurable intervals.
 *
 * @example
 * ```typescript
...
export class DefaultWordListProvider implements WordListProvider {
  ...
  /**
   * Creates a new DefaultWordListProvider instance.
   *
   ...
   * // [
   * //   { word: "badword", parameters: { type: "profanity" } },
   ...
   *
   * This method fetches the word list from the remote URL (either the default
   * or a custom URL specified in the config), parses its content based on the
   * configured format, and updates the internal word lists. It also caches the
   * results if caching is enabled.
   *
   ...
   * - JSON: Parses as JSON array and returns the array
   * - CSV: Splits by newlines and treats each line as a word
   * - Text: Splits by commas, newlines, or whitespace and treats each as a word
   *
